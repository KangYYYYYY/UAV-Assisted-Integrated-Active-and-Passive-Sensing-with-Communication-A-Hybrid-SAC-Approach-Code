...
class Critic(nn.Module):  # According to (s,a), directly calculate Q(s,a)
    def __init__(self, state_dim, para_action_dim, dis_action_dim, hidden_width):
        ...
        # Q2
        self.l4 = nn.Linear(state_dim + para_action_dim, hidden_width)
        self.l5 = nn.Linear(hidden_width, hidden_width)
        self.l6 = nn.Linear(hidden_width, dis_action_dim)
...
class HPSAC(object):
    def __init__(self, state_dim, para_action_dim, dis_action_dim, max_action,env_args):
        ...
        if self.adaptive_alpha:
            ...
            self.target_entropy = -0.5 * para_action_dim
            # We learn log_alpha instead of alpha to ensure that alpha=exp(log_alpha)>0
            self.log_alpha = torch.zeros(1, requires_grad=False).to(device)
    ...
    def choose_action(self, s, deterministic=False):
        ...
        para_a, _ = self.actor(s, deterministic, False)  # When choosing actions, we do not need to compute log_pi
        dis_a_1, dis_a_2 = self.critic(s, para_a)

        Q1 = torch.max(dis_a_1, 1, keepdim=True)[0]
        Q2 = torch.max(dis_a_2, 1, keepdim=True)[0]

        if Q1 > Q2:
            dis_a = dis_a_1.data.cpu().numpy()
        else:
            dis_a = dis_a_2.data.cpu().numpy()

        dis_a = np.argmax(dis_a, axis=1)

        # dis_a = dis_a_1.data.cpu().numpy()
        # dis_a = np.argmax(dis_a, axis=1)

        para_a = para_a.data.cpu().numpy()
    ...
    def learn(self, relay_buffer):
        ...
        with torch.no_grad():
            ...
            target_Q11 = torch.max(target_Q1, 1, keepdim=True)[0]
            target_Q22 = torch.max(target_Q2, 1, keepdim=True)[0]
            target_Q = batch_r + self.GAMMA * (1 - batch_dw) * (torch.min(target_Q11, target_Q22) - self.alpha * log_pi_)
        # Compute current Q
        current_Q1, current_Q2 = self.critic(batch_s, batch_a[:, 1:])
        int_tensor = batch_a[:, 0].int().view(-1, 1).long()
        current_Q11 = current_Q1.gather(1, int_tensor)
        current_Q22 = current_Q2.gather(1, int_tensor)
        # Compute critic loss
        critic_loss = F.mse_loss(current_Q11, target_Q) + F.mse_loss(current_Q22, target_Q)
        # Optimize the critic
        ...
        Q22 = torch.max(Q2, 1, keepdim=True)[0]
        Q = torch.min(Q11, Q22)
